...
The algebra $A$ will be the smooth de Rham complex $(\Omega^\bu(L) , \d_S)$ where $L$ is any smooth manifold. 
...
\deq{
  S_0 =  \beta (\dbar + \d_S) \nu +  \gamma (\dbar + \d_S) \mu +  \beta\partial_\Omega \mu + \frac{1}{2}  \gamma [\mu,\mu] +  \beta[\mu,\nu].
}
...
\deq{
  S_\infty =  \beta (\dbar + \d_S) \nu +  \gamma (\dbar + \d_S) \mu +   \beta \partial_\Omega \mu + \frac12 \frac{1}{1-\nu} (\partial \gamma) \mu^2.
}
...
After parity shifted, we've identified the solutions to the linear equations of motion with triples
\[
...
%\begin{proof}
%We define the map directly. 
%The even part of $E^{hol}(5,10)$ is precisely the Lie algebra of holomorphic divergence-free vector fields on $\CC^5$. 
%We simply embed this in the space $\PV^{1,0}(\CC^5) \hotimes \Omega^0(\RR)$ of $\mu$-fields in the 11-dimensional theory. 
%
%The odd part of $E^{hol}(5,10)$ is the space of holomorphic closed two-forms on $\CC^5$. 
%To embed this in the space of fields of the 11-dimensional theory we need to choose primitives. 
%We do this by fixing a homotopy $H \colon \Omega^{2,hol}_{cl} (\CC^5) \to \Omega^{1,hol} (\CC^5)$ by
%\[
%(H \alpha) (z) = \int_{0}^z \alpha .
%\]
%Since $\alpha$ is closed, one has $\partial H \alpha = \alpha$.
%We send $\alpha$ to the $\gamma$-type field $H \alpha$ viewed as an element of $\Omega^{1,0}(\CC^5) \hotimes \Omega^0(\RR)$. 
%
%All embedded elements of $E(5,10)$ are $\dbar$, $\d_{\RR}$, and $\div$-closed and so descends to a map in cohomology. 
%The first nontrivial Lie bracket compatibility we check is between two $\alpha$-type elements. 
%We will do this in coordinates. 
%Suppose $f^{ij} \d z_i \wedge \d z_j$, $g^{ij} \d z_i \wedge \d z_j$ are two closed two-forms. 
%On one hand, in $E(5,10)$ we have 
%\[
%[f^{ij} \d z_i \wedge \d z_j, g^{kl} \d z_k \wedge \d z_l] = \ep_{ijklm} f^{ij}g^{kl} \partial_{z_m} .
%\]
%
%On the other hand, in the 11-dimensional theory there is the following Lie bracket between $\gamma$-type fields
%\begin{align*}
%[H (f^{ij} \d z_i \wedge \d z_j), H(g^{kl} \d z_k \wedge \d z_l)] & = \big(\del H (f^{ij} \d z_i \wedge \d z_j) \wedge \del H(g^{kl} \d z_k \wedge \d z_l) \big) \vee \partial_{z_1} \wedge \cdots \wedge \partial_{z_5} \\
%& = \ep_{ijklm} f^{ij}g^{kl} \partial_{z_m} 
%\end{align*}
%as desired. 
%
%The final Lie bracket compatibility we need to check is between a divergence-free vector field $f_i \partial_{z_i}$ and a closed two-form $g^{ij} \d z_i \wedge \d z_j$. 
%In $E(5,10)$ we have
%\[
%[f_i \partial_{z_i} , g^{kl} \d z_k \wedge \d z_l] = \del (f_i g^{il} \d z_l) . 
%\]
%This is embedded as the one-form $f_i g^{il} \d z_l$ up to exact terms. 
%On the other hand, in the 11-dimensional theory we have the following $[\mu,\gamma]$ type bracket
%\begin{align*}
%[f_i \partial_{z_i}, H(g^{kl} \d z_k \wedge \d z_l)] & = L_{f_i \partial_{z_i}} H(g^{kl} \d z_k \wedge \d z_l) \\ 
%& = \iota_{f_i \partial_{z_i}} g^{kl} \d z_k \wedge \d z_l + \del \iota_{f_i \partial_{z_i}} H g^{kl} \d z_k \wedge \d z_l \\
%& = f_i g^{il} \d z_l + \del - \text{exact} .
%\end{align*}
%
%
%\end{proof}
