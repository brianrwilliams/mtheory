...
Recall that an $L_\infty$ morphism $\Phi: \cL \rightsquigarrow \cL'$ is the same datum as a map of commutative dg algebras 
\deq{
  \Phi^*: C^\bu(\cL') \to C^\bu(\cL)
}
between their respective Lie algebra cochains. It follows from this that \emph{any} automorphism $\Phi$ of the free commutative algebra $C^\bu(\cL)$ defines a new model of the $L_\infty$ algebra $\cL$, for which the Chevalley--Eilenberg differential is obtained by conjugating $\d_\cL$ by~$\Phi$, and where $\Phi$ itself defines the $L_\infty$ isomorphism.

Let $\cL$ be the sheaf~\eqref{eqn:cplx1} of dg Lie algebras resolving divergence-free vector fields constructed above.
We consider the following automorphism of~$C^\bu(\cL)$, defined by its action on generators:\ingmar{notation for operators vs fields}
\deq[eq:newbase]{
%    \Psi_\infty:  \nu &\mapsto -\frac{\nu}{1-\nu}, \quad \mu \mapsto -\mu \\
    \Psi_\infty: \nu \mapsto 1 - e^{-\nu}, \quad \mu \mapsto e^\nu \mu.
}
This map defines a new model of the local $L_\infty$ algebra, which we will call $\cL_\infty$. The notation refers to the fact that this new model has nonvanishing $L_\infty$ brackets of every order. It is this new model that we will use to define the eleven-dimensional theory of twisted supergravity (as well as the family of analogous formal moduli problems on products of odd Calabi--Yau manifolds with~$\R$). 
...
We apply this construction, taking $\lie{g}$ to be the sheaf of dg Lie algebras resolving divergence-free holomorphic vector fields on a Calabi--Yau manifold $X$, as discussed above, and $A$ to be the smooth de Rham complex $(\Omega^\bu(L) , \d_L)$ where $L$ is any smooth manifold. 
...
\parsec
For concreteness, we recall that the fields of the theory we have constructed on $\C^{2k+1}\times\R$ are given by the sheaf $\cE$ presented in the following diagram:
\begin{equation}
  \label{eq:sympfields} 
  \begin{tikzcd}[row sep = 1 ex]
    -n & -n + 1 & -1 & 0 \\ \hline
    \Omega^{0}(X;L) \ar[r] & \Omega^{1}(X;L) & 
     \PV^{1}(X; L) \ar[r] & \PV^{0}(X; L).
\end{tikzcd}
\end{equation}
We are using the shorthand notation $\PV^{i}(X;L) = (\PV^{i,\bu}(X)\otimes \Omega^*(L), \dbar + \d_{dR}))$, so that the pairing beween $\PV^i(X;L)$ and~$\Omega^i(X;L)$ is of degree $-\dim_\C(X) -\dim_\R(L)$. As such, the $\Z$-grading indicated in~\eqref{eq:sympfields} equips the sheaf of fields with a $(-1)$-shifted symplectic structure, provided that we choose the shift to be given by
\deq{
  n = \dim_\C(X) + \dim_\R(L) - 1.
}
Note, though, that the interactions we consider do not preserve this grading, and as such only define $\Z/2\Z$-graded theories. We denote the fields $(\beta,\gamma,\mu,\nu)$ respectively.

Recall further that we constructed two equivalent descriptions of the BF theory based on divergence-free holomorphic vector fields above. Since we are now in the context of a nondegenerate BV theory, the differential is Hamiltonian, and can be incorporated into the action functional by including a quadratic term; we do this from now on. The first action functional is obtained by considering the BF theory associated to the natural dg Lie structure on the complex modeling divergence-free vector fields (inherited from the Schouten bracket). This takes the form
\deq{
  I_\strict = \beta\partial_\Omega \mu + \frac{1}{2} \gamma [\mu,\mu] + \beta[\mu,\nu].
}
The other action is of infinite order. (In fact, it is related to the BCOV action functional via a procedure we outline below.)\ingmar{or somewhere} Explicitly, this action functional is given by 
\deq{
  I_\infty = \beta \partial_\Omega \mu + \frac{1}{1-\nu} (\partial \gamma) \mu^2.
}

We demonstrated above that the two local $L_\infty$ algebras on which these BF theories are based are equivalent. As such, the BF theories are also equivalent; the map~\eqref{eq:newbase} extends uniquely to a Poisson automorphism of the Lie algebra cochains of the BF theory, which defines an equivalence of cyclic $L_\infty$ algebras. \ingmar{how should we say this} Explicitly, the map is given by
\deq{
  \nu \mapsto 1-e^{-\nu}, \ldots
}
...
Recall that such deformations are governed by the classical master equation: the parameterized family of actions $I + \alpha J$ is sensible only if 
\deq{
  \{I+ \alpha J, I + \alpha J \} = 0.
}
Since this must hold for all $\alpha$, and since the undeformed action $I$ is already a solution to the classical master equation, this reduces to the two conditions 
\deq[eq:2cond]{
  \{I,J\} = \{J,J\} = 0.
}
\begin{thm}
  Let $X$ be a Calabi--Yau manifold of complex dimension $2k+1$ and $L$ a smooth manifold of real dimension $2\ell + 1$, and consider the BV theory $(\cE, I_\infty)$ on $X \times L$ constructed above. The local functional 
  \deq{
    J = \gamma (\del \gamma)^k,
  }
  where $\gamma \in \Omega^1(X;L)$, defines a deformation of~$(\cE,I_\infty)$ as a $\Z/2\Z$-graded BV theory.
\end{thm}
\begin{proof}
