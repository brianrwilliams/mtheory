...
Recall that our conjecture for the space of local operators associated to the holomorphic twist of the six-dimensional worldvolume theory on a stack of two fivebranes is $\Obs_2 (0) \simeq \clie_\bu(\cG_{2,c})(0) \cong \clie_\bu(\cG_{1,c})(0) \otimes \clie_\bu(\til \cG_{2,c})(0)$. 
...
\label{eqn:gk}
\begin{array}{lllll}
g_k (y_i,y,q) \define & q^{3} \left(q^{1 + 3 k/2} \chi^{\lie{sl}(2)}_{k}(q^{-1/2} y)\chi^{\lie{sl}(3)}_{[1,0]}(y_i) + q^{3k/2} \chi^{\lie{sl}(2)}_{k+2}(q^{-1/2} y) \right. \\
&\displaystyle \frac{\left.  - q^{3(k+1)/2} \chi^{\lie{sl}(2)}_{k-1}(q^{-1/2}y) - q^{-1 + 3(k+1)/2} \chi^{\lie{sl}(2)}_{k+1} (q^{-1/2} y) \chi^{\lie{sl}(3)}_{[0,1]}(y_i) \right)}{d(y_i,y,q)} .
\end{array}
\eeqn
