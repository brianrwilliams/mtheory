...
\beqn\label{localLie}
  \cS \define \left(\PV^{1,\bu}(X) \xto{\partial_\Omega} \Omega^{0,\bu}(X) [-1] \right) \; \Hat{\otimes} \; \Omega^{\bu}(M) 
\eeqn
...
%defined on the product $X \times M$ where $X$ is a Calabi--Yau manifold and $M$ is a smooth manifold.
...
%\[
%(\Phi^* J) (\mu^0 + \mu^1 , b^0 \Omega + b^1 \Omega) = \frac16 \int_{X \times L} b^1 \partial b^1 \partial b^1 + \frac16 \int_{X \times L} \mu^0 b^1 \partial b^1 \partial b^1 .
%\]
%
%
...
%  \Tilde{S}_{BF} + J
%\end{equation}
%where $J$ is as in the definition above and $\Tilde{S}_{BF}$ is the action functional for the underlying BF theory which takes the form
%\[
%  \Tilde{S}_{BF} = \int \gamma \vdash \left(\dbar + \d_{\rm dR} + \partial_{\Omega}\right) \mu \wedge \Omega + \frac12 \int \gamma \vdash \{\mu,\mu\}_{\rm NS} \wedge \Omega .
%\]
%We will furthermore decompose $\Tilde{S}_{BF} = S_{\rm free}+I_{BF}$ into its free and interacting pieces.
...
\begin{proof}
  ...
  \end{itemize}
  \surya{signs in last two conditions? comment on why they vanish}
\end{proof}
