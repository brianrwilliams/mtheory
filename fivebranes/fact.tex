...
This weight grading on $\cG$ induces a filtration on the factorization algebra \eqref{eqn:factgrad} and on its $!$-dual $\clie_\bu (\cG_{c})$. 
For $k \geq -1$ define
\beqn
...
\beqn
\cG_N \define \cG / \cG^{(\geq N-1)} .
\eeqn
Similarly, for $N \geq 2$ define
\beqn
\til \cG_N \define \til \cG / \cG^{(N-1)} .
\eeqn
...
\eeqn
%Explicitly, the underlying super vector space of the weight $k$ component $\clie_\bu(\cG_{c})^{(k)}$ is 
%\beqn
%\bigoplus_{m \geq 0} \bigoplus_{\sum_{i=1}^m j_i = k} \left(\cG_c^{(j_1)} \otimes \cdots \otimes \cG_{c}^{(j_m)}\right)_{\Sigma_m} 
%\eeqn
%where we take coinvariants for the symmetric group $\Sigma_m$.
%Similarly, the factorization algebra $\clie_\bu(\til \cG_c) = \oplus_{k \geq 0} \clie_\bu(\til \cG_c)^{(k)}$ is graded.
...
The first term in the filtration \eqref{eqn:fil1} is 
\[
\clie_\bu(\cG_{1,c})
\]
