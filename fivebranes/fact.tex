...
Even in perturbation theory, the full quantum behavior of the holomorphic-topological eleven-dimensional theory is an open question. 
...
In this section we focus on the factorization algebra of classical observables of our prototype for the minimal twist of eleven-dimensional supergravity which we recalled in \S \ref{s:twisted}. 
...
One of the primary features of the factorization algebra $\Bar{\pi}_* \Obs_{sugra}$ is that it is of the form $\clie^\bu(\cG)$ where $\cG$ is a sheaf of $L_\infty$ algebras on $Z$ which is presented as the $C^\infty$-sections of a pro vector bundle.
To get a sense of the construction of this factorization algebra we take a brief excursion to a simpler construction related to branes in five-dimensional Chern--Simons theory. 
...
In this setting, fivebranes which hit the tip of the Taub--NUT become chiral defects supported along a Riemann surface $C$ in the five-manifold. 
In analogy with the situation above, we take the five-manifold to be $\R \times {\rm Tot}(K_C)$ with the fivebranes wrapping the zero section.
Denote by $\Obs_{CS}$ the observables of five-dimensional Chern--Simons theory on $\R \times {\rm Tot}(K_C)$.
Let $\pi \colon \R \times {\rm Tot}(K_C) \to C$ be the natural projection and consider the factorization algebra $\pi_* \Obs_{CS}$ on $C$. 
...
For the rank one abelian gauge group, the space of fields of non-commutative Chern--Simons theory on $\R \times {\rm Tot}(K_C)$ is
\[
...
If, moreover, $U$ is a trivializing cover then
\[
...
Using this notation, the embedding \eqref{eqn:taylor1a} is induced by the Taylor series expansion map $\cO^{hol}(\C) \to \C[[w]]$.
...
For $N > 0$, we define 
\beqn
\til \cG_N \define \cG / \cG^{(\geq N-1)} 
\eeqn
which has the induced structure of a local $L_\infty$ algebra on the worldvolume.

The limit diagram \eqref{eqn:lim} induces a diagram of factorization algebras
\beqn\label{eqn:colim}
\ul\C \to \clie^\bu(\cG_1) \to \clie^\bu(\cG_2) \to \cdots ,
\eeqn
whose colimit is~$\Bar{\pi}_* \Obs_{sugra}$. 
Here $\ul\C$ is the constant factorization algebra whose value is $\C$ on every open set. 
There is a similar diagram upon applying~$!$-duals
\beqn
\ul\C \leftarrow \clie_\bu(\cG_{1,c}) \leftarrow \clie_\bu (\cG_{2,c}) \leftarrow \cdots ,
\eeqn 
