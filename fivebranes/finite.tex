\section{Operators on a finite number of fivebranes} 
...
Concretely, for each $N$ we have constructed a local Lie algebra $\cG_N$ on the worldvolume three-fold $Z$. 
...
The main result of this section is a closed formula for the local character of the factorization algebra $\clie_\bu(\cG_{N,c})$ for $N > 1$, see Theorem \ref{thm:finite}. 
...
The holomorphic twist of the superconformal algebra in dimension six is $\lie{osp}(6|2)$. 
In the case of the holomorphic twist of a six-dimensional superconformal theory we will consider the local character of operators with respect to the subalgebra 
...
We recalled in \S \ref{sec:states} an alternative parametrization of the fugacities in terms of the parameters $y_1,y_2,y_3,y,q$ which satisfy the constraint $y_1 y_2 y_3 = 1$.
...
We deduce the character of the holomorphic twist of the theory on a single fivebrane and will find an exact match with the index of the six-dimensional superconformal theory for the abelian Lie algebra $\lie{gl}(1)$.
...
\beqn
f_{1} (y_i,y,q) = \frac{qy + q^2y^{-1} - q^2(y_1+y_2+y_3) + q^3}{(1-y_1q) (1-y_2 q) (1-y_3 q)} .
\eeqn
...
\parsec
There are various degenerations, or specializations, of this character which are interesting to consider.
A particularly meaningful one is related to two deformations of the theory by elements in the (twisted) superconformal algebra.
...
\]
where $\C^3, \C^2$ are the fundamental $SU(3)$ and $SU(2)$ representations, respectively. 
...
f_{2} (y_i,y,q) = \frac{q^4(y_1+y_2+y_3) + q^2 (y^2 + q + q^2 y^{-2}) - q^{3} (y + q y^{-1})(y_1^{-1} + y_2^{-1} + y_3^{-1})}{(1-y_1q) (1-y_2 q) (1-y_3 q)}.
\eeqn
%\beqn\label{eqn:6dtwo}
...
\brian{add some better transition} 
...
%\begin{align*}
%f_k (y_1,y_2,y_3,y,q) & \define q^{3} \left(q^{1 + 3 (k-2)/2} \chi_{k-2}(q^{-1/2} y)(y_1 + y_2 + y_3) + q^{3(k-2)/2} \chi_k(q^{-1/2} y) \right. \\
%& \left.  - q^{3(k-1)/2} \chi_{k-3}(q^{-1/2}y) - q^{-1 + 3(k-1)/2} \chi_{k-1} (q^{-1/2} y) (y_1^{-1} + y_2^{-1} + y_3^{-1} ) \right) .
%\end{align*}
...
\beqn
\chi_{N}(y_1,y_2,y_3,y,q) = \text{PExp}\left[\sum_{k=2}^N f_k(y_1,y_2,y_3,y,q)\right].
\eeqn
\end{thm}
