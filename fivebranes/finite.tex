...
Recall that our conjecture for the factorization algebra associated to the holomorphic twist of the six-dimensional worldvolume theory associated to a stack of two fivebranes is $\Obs_N \simeq \clie_\bu(\cG_{2,c})$. 
...
\[
\chi_{2} (y_i,y,q) = {\rm PExp} \left[f_2(y_i,y,q) \right] .
\]
where $f_2(y_i,y,q)$ is as in \eqref{eqn:6dtwo}.
\end{conj}
...
\beqn
g_1 (y_i,y,q) = f_1(y_i,y,q)
\eeqn
where $f_1(y_i,y,q)$ is as in equation \eqref{eqn:6done1} and also denote by 
\beqn
g_2 (y_i,y,q) = f_2(y_i,y,q)
\eeqn
where $f_2(y_i,y,q)$ is as in equation \eqref{eqn:6dtwo}. 
Thus $g_2$ is the single particle local character of $\Obs_2 = \clie_\bu(\cG_{2,c})$.
Finally, for $k \geq 3$ let
%\begin{align*}
...
\beqn
\chi_{N}(y_1,y_2,y_3,y,q) = \text{PExp}\left[\sum_{k=2}^N g_k(y_1,y_2,y_3,y,q)\right].
\eeqn
Similarly, the local character of the factorization algebra $\clie_\bu(\til{\cG}_{N,c})$ is 
\beqn
\til{\chi}_{N}(y_1,y_2,y_3,y,q) = \text{PExp}\left[\sum_{k=1}^N g_k(y_1,y_2,y_3,y,q)\right].
\eeqn
\end{thm}
\begin{proof}
...
Thus, it suffices to show that $f_N = \sum_{k = 2}^N g_k$.
Recall that we have the following filtration of the factorization algebra $\clie_\bu(\cG_{N,c})$ given by 
\beqn
\clie_{\bu}(\cG_{2,c}) \leftarrow \clie_{\bu}(\cG_{3,c}) \leftarrow \cdots \clie_\bu (\cG_{N,c}) .
\eeqn
Thus we have the following expression for the associated graded factorization algebra 
\beqn
\text{Gr} \, \clie_{\bu} (\cG_{N,c}) \simeq \bigoplus_{k=2}^N \clie_\bu(\cG^{(k)}.
\eeqn

Thus, it suffices to show that $g_k$ is the single particle index of the factorization algebra $\clie_\bu(\cG_{k,c})$, which is a direct computation.
\end{proof}
...
It follows from the fact that limit of the diagram in \ref{eqn:limit1} is $(\Bar{\pi}_* \Obs_{sugra})^! \simeq \clie_\bu(\cG_c)$ that the large $N$ limit of $\til{\chi}_N$ is precisely the multiparticle supergravity index we computed in Proposition~\ref{prop:sugraindex1}. 
Alternatively, we have the following direct proof of this fact. 

\begin{prop}
One has
\beqn
\chi_{sugra}(y_i, y, q) = \lim_{N \to \infty} \til{\chi}_N(y_i,y,q)
\eeqn
\end{prop}
