...
%\end{align*}
\begin{align*}
g_k (y_i,y,q) & \define q^{3} \left(q^{1 + 3 (k-2)/2} \chi_{k-2}(q^{-1/2} y)(y_1 + y_2 + y_3) + q^{3(k-2)/2} \chi_k(q^{-1/2} y) \right. \\
& \frac{\left.  - q^{3(k-1)/2} \chi_{k-3}(q^{-1/2}y) - q^{-1 + 3(k-1)/2} \chi_{k-1} (q^{-1/2} y) (y_1^{-1} + y_2^{-1} + y_3^{-1} ) \right)}{d(y_i,y,q)} .
\end{align*}

To simplify formulas, we will temporarily denote the single particle character for the $N=1$ theory by 
\beqn
...
Thus $g_2$ is the single particle local character of $\clie_\bu(\cG_{2,c})$ and hence the conjectural single particle index for the superconformal theory associated to the Lie algebra $\lie{sl}(2)$. 
...
From the form of $f_{sugra}$ and $g_k$, one can readily see that the result reduces to observing that
\begin{multline}
