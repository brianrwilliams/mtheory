...
\beqn
g_0 (y_i,y,q) = f_2(y_i,y,q)
\eeqn
where $f_2(y_i,y,q)$ is as in equation \eqref{eqn:6dtwo}. 
Thus $g_2$ is the single particle local character of $\Obs_2 = \clie_\bu(\cG_{2,c}) = \clie_{\bu}(\cG_c^{(0)})$.
...
\beqn
\chi_{N}(y_1,y_2,y_3,y,q) = \text{PExp}\left[\sum_{k=0}^{N-2} g_k(y_1,y_2,y_3,y,q)\right].
\eeqn
Similarly, the local character of the factorization algebra $\clie_\bu(\til{\cG}_{N,c})$ is 
\beqn
\til{\chi}_{N}(y_1,y_2,y_3,y,q) = \text{PExp}\left[\sum_{k=-1}^{N-2} g_k(y_1,y_2,y_3,y,q)\right].
\eeqn
\end{thm}
\begin{proof}
...
Thus, it suffices to show that $f_N = \sum_{k = 0}^{N-2} g_k$.
...
\beqn
\text{Gr} \, \clie_{\bu} (\cG_{N,c}) \simeq \bigoplus_{k=0}^N \clie_\bu(\cG^{(k)}).
\eeqn
