...
\beqn\label{eqn:MA}
\ch\, {M_A(\lambda_1,\lambda_3)} = q^{-\frac32 y} R(q,r,t_1,t_2) \cdot \ch (V_A^{\lambda_1,\lambda_3}) 
\eeqn
...
We thus arrive at the expression for the $j$th term in the sum appearing in equation \eqref{eqn:IA1}:
\beqn\label{eqn:chA1}
\ch M_A(\lambda_1+j,\lambda_3+j)_{t_1=t_2=1} = R(t,r) t^{3 \lambda_3 - 2 \lambda_1} \frac{(\lambda_1+j+1)(\lambda_1+j+2)}{2} t^j \frac{r^{\lambda_3+j}-r^{-\lambda_3-j-2}}{1-r^{-2}} 
\eeqn
where $R(t,r)$ is the specialization at $t_1=t_2=1$ of $R$
\[
R(t,r) = \frac{(1-tr)^3 (1-t r^{-1})^3}{(1-t^2)^3} ,
\]
which we read off readily from Lemma \ref{lem:R}
...
\beqn
\ch I_A(0,1)|_{t_1=t_2=1} = \frac{t^3(r+r^{-1}) + t^6 - 3 t^4}{(1-t^2)^3} .
\eeqn

In the case that $\lambda_1 = 1$ and $\lambda_3 = 0$ we see that this unrefined character simplifies to
\beqn
\ch I_A(1,0)|_{t_1=t_2=1} = \frac{(r^2 + r^{-2} + 1) - 3 t^{-1} (r + r^{-1}) + 3t^{-2} }{(1-t^2)^3} .
\eeqn
\brian{The $(1,0)$ character differs from $N=2$ index by a factor of $t^6 = q^3$ as expected.}
