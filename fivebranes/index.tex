...
\begin{prop}
  ...
  where . The BV pairing is given by

\end{prop}
\begin{proof}
Recall that we have a THF structure on $M$, given by an involutive subsheaf $F_{M}\subset T^{\C}_{M}$, induced from a partially flat conneciton on $\R\oplus W$. The partial connection yields a splitting of the exact sequence

\[
  0 \to F_{M}\cap T_{M/Z}^{\C}\to F_{M}\to \pi^{*}T^{0,1}_{Z}\to 0
\]

Both $F_{M}$ and $\pi^{*}T^{1,0}_{Z}$ are involutive, and the flatness of the connection implies that that the splitting preserves the lie brackets on sections.
\end{proof}
...
\beqn\label{eqn:br1}
I_{M5} = N\int_{M} \thfd_\Omega^{-1}\mu \vee \Omega +\cdots
\eeqn
...
The relevant Maurer-Cartan element is a solution to the equations of motion for the eleven-dimensional theory deformed by this interaction. After deforming with this interaction, varying with respect to $\thfd_{\Omega}^{-1}\mu$ yields the following equation for $\gamma$
\beqn
\bar{\thfd}\thfd \gamma + \thfd_\Omega\left (\frac{\mu}{1-\nu}\right )\wedge \thfd \gamma = N \delta_Z.
\eeqn
...
The phase space at $\infty\in \R_{>0}$ is computed by restricting to a neighborhood of $\infty\in \R_{>0}$ a
\beqn
