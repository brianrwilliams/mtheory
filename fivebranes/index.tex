...
\end{prop}
\begin{proof}
Recall that we have a THF structure on $M$, given by an involutive subsheaf $F_{M}\subset T^{\C}_{M}$, induced from a partially flat conneciton on $\R\oplus W$. The situation is summarized in the diagram below.

\[
  0 \to F_{M}\cap T_{M/Z}^{\C}\to F_{M}\to \pi^{*}T^{0,1}_{Z}\to 0
\]

The existence of a partially flat connection amounts to a choice of splitting $\pi^{*}T^{0,1}_{Z}\to F_{M}$ of the top row. Both $F_{M}$ and $\pi^{*}T^{1,0}_{Z}$ are involutive, and the flatness of the connection implies that that the splitting preserves the lie brackets on sections.
