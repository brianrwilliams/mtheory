...
We consider the equivariant character of local operators of the holomorphic twist of a large number of fivebranes with respect to a finite dimensional subalgebra $\lie{sl}(3)\times \lie{sl}(2)\times \lie {gl}(1)\subset E(5|10)$ which embeds in the following way:

\begin{itemize}
\end{itemize}

This is the image of the subalgebra chosen in the calculation in \ref{} via under the map $E(3|6)\to E(5|10)$ discussed in \surya{earlier}

The equivariant character thus has four fugacities, which we label as follows:
\begin{itemize}
  \item $t_{1}, t_{2}$ again denote generators for the Cartan of $\lie{sl}(3)$ acting by vector fields on $\C^{3}_{z}$.
  \item $r$ denotes a generators for the Cartan of a $\lie{sl}(2)$ acting by the fundamental representation on the normal directions $\C_{w}$.
  \item $q$ denotes a generator for the cartan of additional $\lie{gl}(1)$ acting by a divergence free vector field on $\C^{5}$ that restricts to a dilation on $\C^{3}$.
\end{itemize}

This choice of subalgebra is such that it restricts to the choice made in

The convenient choice of weights is summarized in the following table:
\begin{center}
\begin{tabular}{c c c c c c}
...
\parsec The specialization of this index in \eqref{eqn:special1} yields the plethystic exponential of the following single particle index
\[
...
\parsec The same change of variables in \eqref{eqn:special2} agrees with previously computed indices for single particle states for supergravity on $\rm AdS_{7}\times S^{4}$ \surya{...}
