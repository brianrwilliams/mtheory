...
We will obtain known formulas as a character of the Hilbert space of eleven-dimensional supergravity on $AdS_7\times S^{4}$ in \S\ref{s:states} using our description of twisted supergravity.
...
We propose similar statements at the level of the minimal twist. We fix a complex surface $X$ together with a pair of complex vector bundles $W, M \to X$ of ranks 2 and 1 respectively, satisfying the condition that $\wedge^{3}(W \oplus M) = \det X$. This condition ensures that $Tot (W\oplus M)$ is a Calabi-Yau 5-fold.
...
Let us place this factorization algebra on the three-fold $Tot (M)$. Choosing a fiberwise hermitian metric affords a norm map \[\pi : Tot (M) \to \R_{+}\] whose fibers away from $0$ are the unit $S^{1}$ bundle in $M$. The pushforward $\pi_{*}\Obs_{1}$ is a stratified factoization algebra on the half-line. This may be thought of as a pair consisting of an associative algebra, which may be thought of as the algebra of modes of $\Obs_{1}$ along the five-manifold $Tot (M)$, together with a module for the algebra of modes.

In \cite{SWtensor} it is shown that in the case where $M$ is the trivial bundle, the naive dimensional reduction (where we discard higher Kaluza--Klein modes along the circle fiber) of this factorization algebra agrees with the factorization algebra associated to the minimal twist of five-dimensional $\cN = 2$ Yang--Mills theory on $\R\times M$ for the group $U(1)$ in perturbation theory around the zero instanton sector.
...
\begin{conj}
The mode algebra $\pi_{*}\Obs_{1}|_{\R_{>0}}$ acts on $\cO (\operatorname{Higgs}^{W}_{GL(1)} X)$ where $\operatorname{Higgs}^{W}_{GL(1)} X$ denotes the moduli of $GL(1)$ Higgs bundles on $X$ whose higgs fields are $W[1]$-valued.
\end{conj}
