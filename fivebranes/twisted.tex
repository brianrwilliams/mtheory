...
This theory is obtained from \brian{finish}
...
As a way to highlight the key aspects of the construction, we consider the simplified model of Costello's twisted $M$ theory on $\R \times \C \times C$, where $C$ is a Riemann surface, with some number of twisted `fivebranes' wrapping 
\[
...
We first consider the situation of backreacting some number of (twisted) fivebrans.
Let $Z$ be a three-fold that the fivebranes wrap. 
Let $V\to Z$ a rank 2 holomorphic vector bundle such that $\wedge^{2} V \cong K_{Z}$, and let $X = \text{Tot}(V)$. 
