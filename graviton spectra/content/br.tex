...
More generally, the eleven-dimensional theory can be constructed on any eleven-manifold $\til Z$ equipped with a transversely holomorphic foliation which is equipped with an appropriate holomorphic volume form on the leaves of the foliation.
...
\til Z = \text{Tot} \left(K_M^{1/4} \otimes \C^4 \to M\right) .
\eeqn
...
\til Z = \R \times \text{Tot} \left(K_X^{1/2} \otimes \C^2 \to X \right) .
\eeqn
The M5 branes will wrap the zero section and lie at a point in $\R$.
Notice that in this case the manifold is globally of the form $\R \times Z$ where $Z$ is the Calabi--Yau fivefold above.
\end{itemize}


In \cite{RSW,RWindex} we describe how physical fields in supergravity are manifest in the twisted model.
We review part of this.
...
\end{itemize}
To simplify discussion we point out the explicit form of the fields in the split case~\eqref{eqn:local} where $\til{Z} = \R \times Z$ for $Z$ a Calabi--Yau fivefold.
...
\begin{multline}
S_{BV} = \int_{\til Z}^\Omega \bigg[\beta \wedge (\dbar + \d) \nu + \gamma \wedge (\dbar + \d) \mu +  \beta \wedge \div \mu  \bigg] \\
+ \frac{1}{2} \int_{\til Z}^\Omega  \mu^2 \vee \del \gamma + \int_{\til Z} \gamma \del \gamma \del \gamma + \cdots .
\end{multline}
Here:
\begin{itemize}
...
Together with the BV pairing, then, this first line defines the following linearized BRST complex
\beqn
\begin{tikzcd}
\underline{-} & \underline{+} \\
\cA^\bu(\til Z , \T_{\til Z})_\mu \ar[r,"\div"] & \cA^\bu(\til Z)_\nu \\
\cA^\bu(\til Z)_\beta \ar[r,"\del"] & \cA^\bu(\til Z, \T^*_{\til Z})_\gamma .
\end{tikzcd}
\eeqn
\item We implicitly use the holomorphic volume form in the integrals denoted~$\int^\Omega$.
\item The term 
\beqn\label{eqn:J}
...
\[
\til Z = \operatorname{Tot}(K^{1/4}_M \otimes \C^4\to M)
\]
