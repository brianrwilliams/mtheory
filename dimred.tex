...
\parsec[s:poisson]
The original action for Kodaira-Spencer theory posited by \cite{BCOV}\surya{which reference is BCOV} has a nonlocal kinetic term. In the BV formalism, this is codified by stipulating that the BV pairing is a degenerate odd Poisson tensor rather than an odd symplectic form. The Poisson kernel is given by the expression 
\[
(\div\otimes 1)\delta_{\Delta \subset X\times X} \in \Sym^2 (\PV^{\bu,\bu}(X)[[u]][2])
.\]
...
There is a natural local interaction which equips the above cyclic cochain complex with the structure of $\Z/2$ graded Poisson BV theory. Explicitly, it is given by 
\beqn
I_{BCOV}(\Sigma) = \int^\Omega \langle \exp \Sigma\rangle_0 = \sum_{n\geq 0} \int^\Omega\langle\Sigma^{\otimes n}\rangle_0
\eeqn

where $\langle - \rangle_0$ denotes the genus zero Gromov-Witten invariant with marked points
\beqn
...
\eeqn
 It is easy to see that there is a natural odd Poisson tensor on the above cochain complex such that the inclusion of this subcomplex into the fields of Kodaira-Spencer theory is a Poisson map. There is a natural action functional given by pulling back the Kodaira-Spencer action.
